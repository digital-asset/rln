...
public class BalancesApiImpl {
  ...
  private final RemoteBalanceClient remoteBalanceClient;
  ...
  public BalancesApiImpl(
    ...
    this.remoteBalanceClient = remoteBalanceClient;
  }

  /**
   * walletId should be matched to existing iban of current bank return different types of balance
   * following below: LIQUID: liquid ACTUAL: liquid + locked FUTURE: liquid + incoming
   */
  public AccountBalance getAddressBalance(String address) throws IbanNotFoundException {
    ...
    var accountInfo = accountCache.getAccountInfo(address)
        .orElseThrow(() -> new IbanNotFoundException(address));
    var providerName = setlPartySupplier.getSetlPartyByDamlParty(accountInfo.getProviderParty())
        .getName();
  ...
  public AccountBalance changeBalance(String provider, String address,
      @Valid @NotNull BalanceChange balanceChange) {
    var accountBalance = getAddressBalance(address);
    accountBalance.addLiquid(balanceChange.getChange());
    rlnClient
        .changeBalance(new ChangeBalanceParameters(provider, address, balanceChange.getChange()));
  ...
  public Collection<? extends Balance> getRemoteBalances() {
    return remoteOwnedAddressSupplier
        .getRemoteOwnedAddresses()
        .stream()
        .flatMap(this::getRemoteBalance)
        .collect(Collectors.toList());
  }
