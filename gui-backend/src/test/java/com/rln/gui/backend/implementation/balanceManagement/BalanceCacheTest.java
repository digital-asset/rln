...
class BalanceCacheTest {
  ...
  void GIVEN_several_balance_created_events_WHEN_update_cache_THEN_balance_aggregate_correctly() {
    ...
    var balance1Account1 = BalanceTestUtil
        .createBalance(BalanceTestUtil.IBAN1, BalanceTestUtil.BANK_PARTY,
            BalanceTestUtil.ASSET_CODE1, amount1InAccount1);
    var balance2Account1 = BalanceTestUtil
        .createBalance(BalanceTestUtil.IBAN1, BalanceTestUtil.BANK_PARTY,
            BalanceTestUtil.ASSET_CODE1, amount2InAccount1);
    var balanceAccount2 = BalanceTestUtil
        .createBalance(BalanceTestUtil.IBAN2, BalanceTestUtil.BANK_PARTY,
            BalanceTestUtil.ASSET_CODE2, amountInAccount2);
    ...
    // THEN
    MatcherAssert.assertThat(getBalance(cache, BalanceTestUtil.IBAN1),
        Matchers.is(amount1InAccount1 + amount2InAccount1));
    MatcherAssert
        .assertThat(getBalance(cache, BalanceTestUtil.IBAN2), Matchers.is(amountInAccount2));
  }
  ...
  void GIVEN_balances_in_cache_WHEN_archive_event_comes_THEN_balance_deduct_correctly() {
    // GIVEN
    double amount = 100.0;
    var balance = BalanceTestUtil.createBalance(BalanceTestUtil.IBAN1, BalanceTestUtil.BANK_PARTY,
        BalanceTestUtil.ASSET_CODE1, amount);
    ContractId cid = new ContractId("balance cid");
    BalanceCache<Balance> cache = new LiquidBalanceCache();
    cache.updateBalanceUponCreation(balance, cid);

    MatcherAssert.assertThat(getBalance(cache, BalanceTestUtil.IBAN1), Matchers.is(amount));
